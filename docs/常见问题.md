...
## Windows环境下后端启动爬虫报错NotImplementedError
Q: 在Windows系统上从前端启动爬虫任务时，后端报错 `NotImplementedError` 或 `asyncio.create_subprocess_exec` 失败？<br>
A: 这是Windows系统下asyncio事件循环的已知问题。从v1.0版本开始，backend/main.py已经自动处理了这个问题。如果仍遇到此错误，请确保使用最新版本的代码。技术细节：Windows的默认ProactorEventLoop不支持subprocess操作，需要使用WindowsSelectorEventLoopPolicy。<br>
